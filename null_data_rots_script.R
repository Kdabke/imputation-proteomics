...
#set.seed(5678)
#x <- as.data.frame(sample(c(0,1), replace=TRUE, size=41))
#colnames(x) <- "group"
#x$colname <- colnames(df)
#x <- x[order(x$group),]

#df <- as.data.frame(df[,x$colname])
#tumor_data <- as.data.frame(df[,1:23])
#stroma_data <- as.data.frame(df[,24:41])

#tumor_data[is.na(tumor_data)] <- 0
#stroma_data[is.na(stroma_data)] <- 0

#tumor_data <- tumor_data[apply(tumor_data == 0, 1, sum) >= 21, ]
#stroma_data <- stroma_data[apply(stroma_data == 0, 1, sum) >= 16, ]

#df <- filter(df, !rownames(df) %in% rownames(tumor_data))
#df <- filter(df, !rownames(df) %in% rownames(stroma_data))

set.seed(5678)
rots <- ROTS(df, groups = c(sample(c(0,1), replace=TRUE, size=41)),
